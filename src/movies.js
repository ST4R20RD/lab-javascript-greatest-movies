...
function getAllDirectors(movieArr) {
  const directors = movieArr.map(function (currentMovie) {
...
function howManyMovies(movieArr) {
  const spielbergMovies = movieArr.filter(function (currentMovie) {
    return currentMovie.director == 'Steven Spielberg' && currentMovie.genre.includes('Drama')
  })
  return Object.keys(spielbergMovies).length;
};
...
function scoresAverage(movieArr) {
  ...
  const averageScore = movieArr.reduce(function (accumulator, currentValue){
    return accumulator + Number(currentValue.score)
  },0)
  return parseFloat((averageScore/movieArr.length).toFixed(2))
}

// Iteration 4: Drama movies - Get the average of Drama Movies
function dramaMoviesScore(movieArr) {
  return scoresAverage(movieArr.filter(film => film.genre.includes('Drama'))); 
}

// Iteration 5: Ordering by year - Order by year, ascending (in growing order)
function orderByYear(movieArr) {
  const orderMovies = movieArr.sort((movieYear1, movieYear2) => {
...
function orderAlphabetically(movieArr) {
  ...
  const onlyTitle = movieArr.map((movieArr) => movieArr.title);
...
function turnHoursToMinutes(movieArr) {
  const moviesDuration = movieArr.map((timeMovie) => timeMovie.duration);
